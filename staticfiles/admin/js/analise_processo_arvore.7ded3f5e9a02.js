...
function formatCnjDigits(raw) {
         ...
         * ======================================================= */
            ...
            if (cardIndex !== null) {
	            ...
	            const findCumprimentoQuestionKey = () => {
	                if (treeConfig && treeConfig['cumprimento_de_sentenca']) {
	                    return 'cumprimento_de_sentenca';
	                }
	                const keys = Object.keys(treeConfig || {});
	                const foundByLabel = keys.find(k => {
	                    const q = treeConfig[k];
	                    if (!q) return false;
	                    const text = String(q.texto_pergunta || '').toLowerCase();
	                    if (!text.includes('cumprimento') || !text.includes('senten')) return false;
	                    return q.tipo_campo === 'OPCOES';
	                });
	                if (foundByLabel) return foundByLabel;
	                const foundByKey = keys.find(k => String(k).toLowerCase().includes('cumprimento'));
	                return foundByKey || null;
	            };

	            const cumprimentoKey = findCumprimentoQuestionKey();
	            const $cumprimentoField = cumprimentoKey
	                ? $scope.find(`select[name="${prefix}${cumprimentoKey}"]`)
	                : $();

	            const isIniciarCsOption = (optEl) => {
	                const value = String(optEl?.value || '').toUpperCase();
	                const text = String(optEl?.textContent || '').toUpperCase();
            ...
            cincoAnosAtras.setFullYear(cincoAnosAtras.getFullYear() - 5);
	            ...
	            if (dataSelecionada < cincoAnosAtras) {
	                ...
	                if ($cumprimentoField.length) {
	                    ...
	                    if (selectedOpt && isIniciarCsOption(selectedOpt)) {
         ...
         * ======================================================= */
	        ...
	        function renderQuestion(questionKey, $container, currentResponses, cardIndex = null) {
	            ...
	            const isIniciarCsOption = (text) => {
	                ...
	                const isIniciar = normalized.includes('INICI');
	                const isHabilitar = normalized.includes('HABILIT');
	                return normalized.includes('CS') && (isIniciar || isHabilitar);
	            };
