...
function formatCnjDigits(raw) {
        ...
        function renderQuestion(questionKey, $container, currentResponses, cardIndex = null) {
            ...
            if (question.tipo_campo === 'PROCESSO_VINCULADO') {
                ...
                if (autoNext) {
                    if (!treeConfig[autoNext]) {
                        const nextLabel =
                            question.proxima_questao_texto ||
                            (autoNextFromOptions ? autoNextFromOptions.proxima_questao_texto : '') ||
                            autoNext;
                        $container.append(
                            `<p class="errornote">Próxima questão configurada (${nextLabel}) não existe neste Tipo de Análise.</p>`
                        );
                        return;
                    }
                    renderQuestion(autoNext, $container, currentResponses, cardIndex);
                }
                return;
            }
        ...
        function renderNextQuestion(
            ...
            if (nextQuestionKey) {
                if (!treeConfig[nextQuestionKey]) {
                    const option = (currentQuestion.opcoes || []).find(
                        opt => opt && opt.texto_resposta === selectedResponseText
                    );
                    const nextLabel =
                        currentQuestion.proxima_questao_texto ||
                        (option ? option.proxima_questao_texto : '') ||
                        nextQuestionKey;
