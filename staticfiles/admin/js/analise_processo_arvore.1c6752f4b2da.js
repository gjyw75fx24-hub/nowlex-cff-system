...
function showCffSystemDialog(message, type = 'warning', onClose = null) {
        ...
        function showEditModeIndicator(cnj, cardIndex) {
            ...
            if (cardTipoId && cardTipoId !== currentTipoId) {
                fetchDecisionTreeConfig({ tipoId: cardTipoId, forceReload: false })
                    .done(proceedRestore)
                    .fail(() => {
...
function formatCnjDigits(raw) {
        ...
        function fetchDecisionTreeConfig() {
            ...
            const cacheKey = tipoId ? `${DECISION_TREE_CACHE_KEY}:${tipoId}` : DECISION_TREE_CACHE_KEY;
...
function renderMonitoriaContractSelector(question, $container, currentResponses, cardIndex = null, onSelectionChanged = null) {
         ...
         * ======================================================= */
		        ...
		        let analysisSelectionObserver = null;
        ...
        const scheduleAnalysisBoot = (force = false) => {
	        ...
	        const resetDecisionTreeState = () => {
		        ...
		        const enforceSelectionGate = () => {
		            const container = $dynamicQuestionsContainer.get(0);
		            if (!container) {
		                return;
		            }
		            if (analysisSelectionObserver) {
		                analysisSelectionObserver.disconnect();
		                analysisSelectionObserver = null;
		            }
		            analysisSelectionObserver = new MutationObserver(() => {
		            ...
		            analysisSelectionObserver.observe(container, { childList: true });
		        };
            ...
            fetchAnalysisTypes()
                .done(types => promptSelectAnalysisType(types))
                .done(selectedType => {
	                    $dynamicQuestionsContainer.html('<p>Carregando tipo de análise...</p>');
		                    fetchDecisionTreeConfig({ tipoId: selectedType.id, forceReload: false })
		                        .done(() => {
		                        ...
		                        .fail(() => {
		                            ...
		                            updateActionButtons();
		                            $dynamicQuestionsContainer.html(
		                                '<p class="errornote">Erro ao carregar o Tipo de Análise selecionado.</p>'
		                            );
	                ...
	                .fail(() => {
	                    ...
	                    updateActionButtons();
	                    $dynamicQuestionsContainer.html(
	                        '<p class="errornote">Não foi possível carregar os Tipos de Análise.</p>'
	                    );
	                    $startAnalysisButton.prop('disabled', false);
	                });
		        });
